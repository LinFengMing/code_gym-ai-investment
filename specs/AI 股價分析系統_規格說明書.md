...
2. 繪製專業的K線圖、移動平均線圖表和RSI相對強弱指標圖表
...
4. `calculate_rsi(df, period=14)` - 計算RSI相對強弱指標（預設14天，支援自訂參數）
5. `generate_ai_insights(symbol, stock_data)` - 使用OpenAI進行技術分析（需包含RSI數據）

**主程式邏輯**:
1. 設置頁面標題和側邊欄輸入控制項（包含RSI計算天數輸入）
2. 實施輸入驗證和錯誤處理（包含RSI參數驗證）
3. 獲取和處理股票數據
4. 計算移動平均線和RSI指標
5. 繪製互動式K線圖和移動平均線（上方區域）
6. 繪製RSI相對強弱指標圖表（下方區域，包含超買超賣區域標記）
7. 顯示基本統計資訊
8. 執行AI技術分析（包含RSI分析內容）
9. 顯示歷史數據表格（包含RSI欄位）
